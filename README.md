...
- **`initGame()`****:** Função principal que será chamada para configurar o jogo. Isso inclui:
...
- **`loadImage(path)`****:** Uma função auxiliar para carregar uma imagem e retornar uma Promise. Isso garante que todas as imagens estejam prontas antes do jogo começar.

- **`loadAllAssets()`****:** Orquestra o carregamento de todas as imagens do dinossauro, cactos, chão, nuvens e botão de reiniciar.
...
- **`gameLoop()`****:** A função principal que será chamada repetidamente (usando `requestAnimationFrame`). Dentro dela:
...
- **`checkCollision(dino, cactus)`****:** Uma função que verifica se as caixas delimitadoras (hitboxes) do dinossauro e de um cacto se sobrepõem. Se sim, o jogo termina.

## 7. Gerenciamento de Obstáculos

- **`generateCactus()`****:** Cria um novo objeto `Cactus` em uma posição aleatória na borda direita da tela.

- **`removeOffscreenCactus()`****:** Remove cactos que já passaram da tela para otimizar o desempenho.

## 8. Pontuação e Game Over

- **`updateScore()`****:** Incrementa a pontuação a cada quadro ou a cada certa distância percorrida.
...
- **`handleKeyDown(event)`****:** Função que responde a eventos de teclado (por exemplo, a tecla `Espaço` para pular).

- **`restartGame()`****:** Reseta todas as variáveis de estado e inicia o `gameLoop` novamente.
